--Created using senpaizuri's Puzzle Maker (updated by Naim & Larry126)
--Partially rewritten by edo9300
Debug.ReloadFieldBegin(DUEL_ATTACK_FIRST_TURN+DUEL_SIMPLE_AI,5)
Debug.SetPlayerInfo(0,8000,0,0)
Debug.SetPlayerInfo(1,8000,0,0)

--클레어
c=Debug.AddCard(10050110,0,0,LOCATION_EMZONE,1,POS_FACEUP_ATTACK)
tc=Debug.AddCard(10050114,1,1,LOCATION_EMZONE,1,POS_FACEUP_ATTACK)

Debug.AddCard(10053531,1,1,LOCATION_MZONE,2,POS_FACEUP_ATTACK)
Debug.AddCard(10053531,1,1,LOCATION_MZONE,2,POS_FACEUP)
Debug.AddCard(10053531,1,1,LOCATION_MZONE,2,POS_FACEUP)
Debug.AddCard(10053531,1,1,LOCATION_MZONE,2,POS_FACEUP)
Debug.AddCard(10053531,1,1,LOCATION_MZONE,2,POS_FACEUP)
Debug.AddCard(10053531,1,1,LOCATION_MZONE,2,POS_FACEUP)
Debug.AddCard(10053531,1,1,LOCATION_MZONE,2,POS_FACEUP)
Debug.AddCard(10053531,1,1,LOCATION_MZONE,2,POS_FACEUP)

Debug.AddCard(10051231,1,1,LOCATION_MZONE,0,POS_FACEUP_ATTACK)
Debug.AddCard(10051231,1,1,LOCATION_MZONE,0,POS_FACEUP)
Debug.AddCard(10051231,1,1,LOCATION_MZONE,0,POS_FACEUP)
Debug.AddCard(10051231,1,1,LOCATION_MZONE,0,POS_FACEUP)
Debug.AddCard(10051231,1,1,LOCATION_MZONE,0,POS_FACEUP)
Debug.AddCard(10051231,1,1,LOCATION_MZONE,0,POS_FACEUP)
Debug.AddCard(10051231,1,1,LOCATION_MZONE,0,POS_FACEUP)
Debug.AddCard(10051231,1,1,LOCATION_MZONE,0,POS_FACEUP)

Debug.AddCard(10052321,1,1,LOCATION_MZONE,4,POS_FACEUP_ATTACK)
Debug.AddCard(10052321,1,1,LOCATION_MZONE,4,POS_FACEUP)
Debug.AddCard(10052321,1,1,LOCATION_MZONE,4,POS_FACEUP)
Debug.AddCard(10052321,1,1,LOCATION_MZONE,4,POS_FACEUP)
Debug.AddCard(10052321,1,1,LOCATION_MZONE,4,POS_FACEUP)
Debug.AddCard(10052321,1,1,LOCATION_MZONE,4,POS_FACEUP)

Debug.AddCard(10054212,1,1,LOCATION_SZONE,1,POS_FACEUP)
Debug.AddCard(10055311,1,1,LOCATION_SZONE,3,POS_FACEUP)

Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,0,0,LOCATION_DECK,0,POS_FACEDOWN)

Debug.AddCard(10054621,1,1,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,1,1,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,1,1,LOCATION_DECK,0,POS_FACEDOWN)
Debug.AddCard(10054621,1,1,LOCATION_DECK,0,POS_FACEDOWN)

Debug.AddCard(10053411,0,0,LOCATION_MZONE,2,POS_FACEUP_ATTACK)
Debug.AddCard(10053411,0,0,LOCATION_MZONE,2,POS_FACEUP)
Debug.AddCard(10053411,0,0,LOCATION_MZONE,2,POS_FACEUP)
Debug.AddCard(10053411,0,0,LOCATION_MZONE,2,POS_FACEUP)

Debug.AddCard(10054421,0,0,LOCATION_HAND,0,POS_FACEDOWN)
Debug.AddCard(10054321,0,0,LOCATION_HAND,0,POS_FACEDOWN)
Debug.AddCard(10052221,0,0,LOCATION_HAND,0,POS_FACEDOWN)
Debug.AddCard(10052331,0,0,LOCATION_HAND,0,POS_FACEDOWN)

Debug.ReloadFieldEnd()

Debug.ShowHint[[★후다 유미네
자아... 믿는 겁니다
부정할 수 없는 BL<이노센트 보이즈 러브>에
이끌려온 사람들이여...]]
Debug.ShowHint[[★후다 유미네
아무것도 부끄러워할 필요 없습니다.
곧 모든 남자가 남자와 사귀는 세계가 도래할 것입니다.
BL이야말로 세계의 정도]]
Debug.ShowHint[[★후다 유미네
자, 있는 그대로의 최애를 향한 생각을 풀어내는 것입니다
이 유미네 교단에서 동인지를 발산해 이 에토와리아를
누구나가 사랑에 정직하게 될 수 있는 세계를 만드는겁니다]]
Debug.ShowHint[[이번 턴 안에 상대 배틀 크리에메이트를 전부 쓰러트리세요]]

local e0=Effect.CreateEffect(tc)
e0:SetType(EFFECT_TYPE_SINGLE)
e0:SetCode(EFFECT_CANNOT_TRIGGER)
tc:RegisterEffect(e0)
local e5=Effect.CreateEffect(c)
e5:SetType(EFFECT_TYPE_SINGLE)
e5:SetCode(EFFECT_CANNOT_TRIGGER)
c:RegisterEffect(e5)
local e1=Effect.CreateEffect(c)
e1:SetType(EFFECT_TYPE_SINGLE)
e1:SetCode(EFFECT_UPDATE_DEFENSE)
e1:SetProperty(EFFECT_FLAG_CANNOT_DISABLE)
e1:SetReset(RESET_EVENT+RESETS_STANDARD)
e1:SetValue(10)
c:RegisterEffect(e1)
local e2=Effect.CreateEffect(tc)
e2:SetType(EFFECT_TYPE_SINGLE)
e2:SetCode(EFFECT_UPDATE_DEFENSE)
e2:SetProperty(EFFECT_FLAG_CANNOT_DISABLE)
e2:SetReset(RESET_EVENT+RESETS_STANDARD)
e2:SetValue(10)
tc:RegisterEffect(e2)

local e3=Effect.CreateEffect(c)
e3:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
e3:SetProperty(EFFECT_FLAG_IGNORE_IMMUNE+EFFECT_FLAG_CANNOT_DISABLE)
e3:SetRange(LOCATION_MZONE)
e3:SetCode(EVENT_ADJUST)
e3:SetOperation(function (e,tp,eg,ep,ev,re,r,rp)
local g=Duel.GetMatchingGroup(Kirafan6.NoEmFzonefilter,tp,LOCATION_MZONE,LOCATION_MZONE,nil)
local tc=g:GetFirst()
	for tc in aux.Next(g) do
	local e1=Effect.CreateEffect(tc)
	e1:SetType(EFFECT_TYPE_SINGLE)
	e1:SetProperty(EFFECT_FLAG_SINGLE_RANGE)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCode(EFFECT_SELF_DESTROY)
	e1:SetCondition(function (e)
	if e:GetHandler():GetCounter(0xb01)>0 then return e:GetHandler():GetDefense()<=1 end
	return e:GetHandler():GetDefense()==0 end)
	tc:RegisterEffect(e1)
	end
end)
c:RegisterEffect(e3)

local e4=Effect.CreateEffect(c)
e4:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
e4:SetProperty(EFFECT_FLAG_DELAY)
e4:SetRange(LOCATION_EMZONE)
e4:SetCountLimit(1)
e4:SetCode(EVENT_TO_GRAVE)
e4:SetOperation(function (e,tp,eg,ep,ev,re,r,rp)
local g=Duel.GetMatchingGroup(nil,tp,LOCATION_GRAVE,0,nil)
Duel.SendtoDeck(g,nil,-2,REASON_RULE)
end)
c:RegisterEffect(e4)

local e9=Effect.CreateEffect(c)
e9:SetCategory(CATEGORY_DECKDES)
e9:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
e9:SetCode(EVENT_PHASE+PHASE_STANDBY)
e9:SetProperty(EFFECT_FLAG_CANNOT_DISABLE)
e9:SetRange(LOCATION_EMZONE)
e9:SetCountLimit(1)
e9:SetCondition(Kirafan.Dottecon)
e9:SetOperation(function (e,tp,eg,ep,ev,re,r,rp)
if Duel.SelectYesNo(tp,aux.Stringid(10050111,0)) then
local g=Duel.SelectMatchingCard(tp,nil,tp,LOCATION_HAND+LOCATION_MZONE,0,0,1,e:GetHandler())
if #g==0 then
else
local tc=g:GetFirst()
if tc:IsLocation(LOCATION_MZONE) then
local tg=tc:GetOverlayGroup()
Duel.Remove(tg,POS_FACEUP,REASON_RULE)
Duel.Remove(tc,POS_FACEUP,REASON_RULE)
Duel.SendtoGrave(tc,REASON_RULE)
else
Duel.SendtoGrave(tc,REASON_RULE) end end end
end)
Duel.RegisterEffect(e9,0)

local e4=Effect.GlobalEffect()
	e4:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
	e4:SetCode(EVENT_PHASE+PHASE_BATTLE)
	e4:SetCountLimit(1)
	e4:SetOperation(function(e,tp,eg,ep,ev,re,r,rp)
if Duel.GetMatchingGroupCount(nil,tp,0,LOCATION_MZONE,nil)==1 then
Debug.ShowHint[[★이케타니 노노
저기 죄송한데요, 타마키 선배
후다후다 선배랑 만든 BL 게임에서
'젊디젊은 남자 둘이 화장실에서 뭔가를...']]
Debug.ShowHint[[★이케타니 노노
이게 BL 묘사인가요?
화장실에서 뭘 하나요?]]
Debug.ShowHint[[★혼다 타마키
묻지마!!!!....
나도 잘은 모르지만 남자 둘이서...
소곤소곤]]
Debug.ShowHint[[★이케타니 노노
이, 이런 저속한 게임 못 팔아요!!!
집에 갈래요!!]]
Duel.SetLP(1-tp,0)
else
Debug.ShowHint[[★소라
대어 대어~♪ 어떤 동인지도 모두 훌륭하네요
하지만 유미네 선생님 건 아르시브가 보면 큰일이니
어딘가에 숨겨둬야...]]
Debug.ShowHint[[★아르시브
소라님!
또, 동인지를 읽고 있습니까?
책이 어지러져있고 남들에게 본보기가... 음?]]
Debug.ShowHint[[★아르시브
...........]]
Debug.ShowHint[[★소라
아르시브?
무슨 일이니...?
아르시브~~~!!]]
Debug.ShowHint[[필두신관 아르시브는 순수하트의 소유자였습니다
그녀는 동인지의 내용을 받아들이지 못하고 실신
그것에 대한 기억도 잃어버렸다고 합니다]]
Debug.ShowHint[[그리고 아르시브를 무참히 녹아웃한 여신님의 동인지는
신전 금서고에 봉인되어
필두신관 고로시(아르시브 슬레이어)라고 불려 구전되었고
신전에 봉인되어 최종병기로서 전설이 되었다고 합니다]]
Duel.SetLP(tp,0) end
end)
Duel.RegisterEffect(e4,0)

